class SimpleOpeningHours { 
   ...
   * Split on ;
   */
  splitHard = function (inp) {
      return inp.split(';');
  };
  
  parseHardPart = function (part) {
      var _this = this;
      if (part === "24/7") {
   ...
   */
  calcRange = function (min, max, maxval) {
      if (min === max) {
          return [min];
      }
      var range = [min];
...
export default SimpleOpeningHours;